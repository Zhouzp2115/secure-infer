...
class LeNet(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(3, 6, 5, stride=1, padding=0 ,bias=False),
            nn.ReLU(True),
            nn.MaxPool2d(2, 2),
...
def train(model, train_loader, device, batch_size):
    ...
    for epoch in range(20):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
           if((batch_idx + 1) * batch_size > 50000):
              break
...
def save(model):
    model = model.cpu()
    file = open('../bin/DATA/LeNet-CIFAR10/weight.bin', 'wb')
    ...
    weight = model.fc[4].weight.detach()
    print('fc3')
    print(weight.max())
    print(weight.min())
    weight = weight.reshape(-1).numpy().tolist()
